...
import { AllDayEventsModal } from "./services/all-day-events-modal";
...
export const CalendarReactView: React.FC<CalendarReactViewProps> = ({
  ...
  const computedEmbedCalendarHeight = containerHeight > 0
    ? Math.max(320, Math.round(containerHeight - 76))
    : embedFallbackHeight;
  ...
  const handleMoreLinkClick = useCallback((arg: any) => {
    if (!app) return;

    const date = arg.date;
    const clickedDateStart = new Date(date);
    clickedDateStart.setHours(0, 0, 0, 0);
    const clickedDateEnd = new Date(date);
    clickedDateEnd.setHours(23, 59, 59, 999);

    // console.log("More link clicked for date:", date);
    // console.log("Current entries:", entries.length);
    const dayEvents = entries.filter((entry) => {
      const entryStart = new Date(entry.startDate);
      const entryEnd = entry.endDate ? new Date(entry.endDate) : new Date(entryStart);

      // Strict day checking can be tricky with timezones. 
      // FullCalendar returns a local date for the clicked day.
      // We should check if the event *overlaps* this day.

      return (entryStart.getTime() <= clickedDateEnd.getTime()) &&
        (entryEnd.getTime() >= clickedDateStart.getTime());
    });
    // console.log("Found day events:", dayEvents.length);

    const allDayEvents = dayEvents.filter(e => {
      if (e.isExternal) {
        return !!e.externalEvent?.isAllDay;
      }
      const allDaySource = allDayProperty
        ? tryGetValue(e.entry, allDayProperty)
        : null;
      const normalizedAllDaySource = normalizeValue(allDaySource).trim().toLowerCase();
      return ["true", "yes", "y", "1"].includes(normalizedAllDaySource);
    });

    if (allDayEvents.length > 0) {
      new AllDayEventsModal(app, date, allDayEvents, onEntryClick).open();
    } else {
      new AllDayEventsModal(app, date, dayEvents, onEntryClick).open();
    }

    // Return "popover" or similar might be needed if fullcalendar requires it, 
    // but usually void is fine for custom handling. Check logic.
    return "popover"; // Trying explicit return again as void might be swallowing the event?
  }, [app, entries, allDayProperty, onEntryClick]);
  ...
  }, [onDateChange, currentDate, viewMode, targetDayCount]);
  ...
  const handleTodayCentered = useCallback(() => {
    ...
    setIsFollowingNow(true);
    setTimeout(() => scrollToNow(), 50);
  }, [targetDayCount, viewMode, onDateChange, scrollToNow]);
  ...
  }, [resolvedNavDays, viewMode, onDateChange, targetDayCount]);
  ...
  return (
    <div
      ...
      style={{
        ...
        "--tps-allday-max-rows": `${allDayMaxRows ?? 3}`,
        ...
        >
          {viewMode !== 'continuous' && (
            <FullCalendar
              ...
              dayMaxEvents={(allDayMaxRows != null ? allDayMaxRows : 4)}
              // @ts-ignore
