...
export class CalendarView extends BasesView {
  ...
  private newEventTypeFolder: string | null = null;
  ...
  private updateBasesHeaderOffset(): void {
    ...
    // Safety: embedded panes can briefly mount at header-only height; enforce a bounded fallback.
    const measuredHeight = Math.max(0, Math.round(this.containerEl.getBoundingClientRect().height));
    if (isEmbedded) {
      if (measuredHeight > 0 && measuredHeight < 220) {
        this.containerEl.style.minHeight = '420px';
      } else if (this.containerEl.style.minHeight) {
        this.containerEl.style.removeProperty('min-height');
      }
    } else if (this.containerEl.offsetHeight === 0) {
  ...
  private loadConfig(): void {
    ...
    console.log("[DEBUG-CALENDAR-V2] loadConfig property resolution:", {
      startPropConfig: startProp,
      startPropFinal: this.startDateProp,
      endPropConfig: endProp,
      endPropFinal: this.endDateProp
    });
    ...
    // Filter-based auto view mode switching
    const filterRangeAutoValue = this.config.get("filterRangeAuto");
    this.filterRangeAuto = filterRangeAutoValue === "true" || filterRangeAutoValue === true;

    // Context-aware date detection (for embedding in daily notes)
    const contextDateEnabledValue = this.config.get("contextDateEnabled");
    this.contextDateEnabled = contextDateEnabledValue === "true" || contextDateEnabledValue === true;
    ...
    const tpsTargetFolder = (this.config.get("tpsTargetFolder") as string) || null;
    const tpsTypeFolder = (this.config.get("tpsTypeFolder") as string) || tpsTargetFolder;
    ...
    const baseTypeConfigured = !!(filterDefaults.folderPath || tpsTypeFolder);
    this.newEventTypeFolder = baseTypeConfigured
      ? null
      : (this.config.get("newEventTypeFolder") as string) || (this.config.get("newEventFolder") as string) || null;
    ...
    if (this.plugin.autoCreateService) {
      this.plugin.autoCreateService.updateConfig({
        ...
        canceledStatusValue: this.plugin.settings.canceledStatusValue?.trim() || null,
      });
    }
  ...
  private updateNewEventService(): void {
    ...
    this.newEventService.updateConfig({
      ...
      folderPath: this.newEventTypeFolder,
  ...
  public async updateCalendar(): Promise<void> {
    ...
    for (const file of files) {
      ...
      if (statusFieldName) {
        const statusValue = fm[statusFieldName];
        if (statusValue) {
      ...
      const eventId = fm[this.plugin.settings.eventIdKey]
        ? String(fm[this.plugin.settings.eventIdKey])
        : "";
    ...
    for (const entry of queryData.data) {
      ...
      if (startDate) {
        ...
        const eventIdForMatch = cache?.frontmatter?.[this.plugin.settings.eventIdKey]
          ? String(cache.frontmatter[this.plugin.settings.eventIdKey])
          : undefined;
        ...
        if (eventIdForMatch) {
          ...
          // No event ID, try fuzzy match by Title + Start Time
    ...
    for (const extEvent of allExternalEvents) {
      ...
      const isSuppressed = suppressedExternalEventIds.has(extEvent.id);
      if (isSuppressed) {
        continue;
      }
    ...
    // If filterRangeAuto is enabled, compute date range and auto-switch view mode
    if (this.filterRangeAuto) {
      this.computeFilterDateRange(finalEntries);
    }
  ...
  /**
   * Computes the date range from filtered entries and auto-switches view mode.
   * If entries span 7 days or less → timeline view (3d/5d/7d)
  ...
  private computeFilterDateRange(entries: CalendarEntry[]): void {
    ...
    // Set currentDate to the start of the range so the calendar displays filtered events
    // Center the view on the first event for timeline views
    if (this.viewMode !== "month") {
      // For timeline views, set currentDate to the min date
    ...
    if (this.isTemplateFile(file)) {
      try {
        ...
        const created = await this.newEventService.createEvent(start, end, undefined, {
          ...
          folderOverride: this.getFilterCreationDefaults().folderPath,
  ...
  private collectFilterConditions(filters: unknown): Array<{ property: string; operator: string; value: unknown }> {
    ...
    const visit = (node: any) => {
      ...
      const property = String((node as any).property || (node as any).field || "").trim();
      if (!property) return;
      let value = (node as any).value ?? (node as any).pattern ?? (node as any).match;
      if (value && typeof value === "object" && "value" in value) {
        value = (value as any).value;
      }
      const operator = String((node as any).op || (node as any).operator || "").trim();
    ...
    const options: ViewOption[] = [
      ...
      },
      {
        ...
        items: [
          ...
          {
            displayName: "Auto view mode (from filter range)",
            type: "dropdown",
            key: "filterRangeAuto",
            default: "false",
            options: {
              true: "Auto (based on filtered dates)",
              false: "Manual (use selected view mode)",
            },
          },
          {
            displayName: "Context date (from note title)",
            type: "dropdown",
            key: "contextDateEnabled",
            default: "false",
            options: {
              true: "Enabled (detect date from parent note)",
              false: "Disabled",
            },
          },
      ...
      },
      {
        ...
        items: [
          {
            displayName: "Type folder (override)",
            type: "text",
            key: "newEventTypeFolder",
            default: "",
            placeholder: "Folder path used as event type",
          },
        ],
      },
  ...
  private static getExternalCalendarViewOptions(plugin?: CalendarPluginBridge): any[] {
    const calendars = plugin?.settings?.externalCalendars ?? [];
  ...
  private async linkExistingNoteToEvent(eventFile: TFile, parentFile: TFile): Promise<void> {
    try {
      ...
      if (doBidirectional) {
        ...
        await this.app.fileManager.processFrontMatter(eventFile, (fm) => {
          fm[parentKey] = `[[${parentFile.basename}]]`;
        });
      }
