...
export class NewEventService {
  ...
  async createEvent(
    ...
    try {
      ...
      // Prompt for Parent Link if enabled (skip if Link Existing already chose parent)
      // DISABLED per user request: "Create" button should create immediate unlinked event
      ...
      const folderPath = this.resolveFolderPath(
        this.pendingTypeFolderPath ?? options?.typeFolderOverride ?? options?.folderOverride,
      );
      ...
      if (templateFile) {
        ...
        if (!options?.useBaseDefaults) {
          ...
          await this.config.app.fileManager.processFrontMatter(file, (fm) => {
            ...
            // Store status from overrides before Object.assign (in case template has status)
            const statusOverride = frontmatter.status;
            ...
            // Ensure status from overrides is applied last
            if (statusOverride !== undefined) {
              this.setFrontmatterValueCaseInsensitive(fm, "status", statusOverride);
            }
          });

          if (frontmatter.status !== undefined) {
            this.scheduleStatusReapply(file, frontmatter.status);
          }
        }

        // Invoke Templater plugin to process <% tp.* %> commands in the file
        if (templateFile) {
          const templater = (this.config.app as any)?.plugins?.plugins?.["templater-obsidian"];
          if (templater?.templater) {
            try {
              ...
              logger.log(`[NewEventService] Templater processing complete for: ${file.basename}`);
            } catch (e) {
              logger.warn(`[NewEventService] Templater processing failed for "${file.basename}":`, e);
            }
          }
        }
        ...
        if (options?.useBaseDefaults) {
          ...
          // Re-apply the full event frontmatter after post-creation hooks.
          // Linter can strip/normalize keys, so we restore our intended values.
          this.scheduleFrontmatterReapply(file, frontmatter);
          if (frontmatter.status !== undefined) {
            this.scheduleStatusReapply(file, frontmatter.status);
          }
        }
        ...
        // No template - create with basic frontmatter (or empty if base defaults should apply)
        const content = options?.useBaseDefaults ? "" : this.buildNoteContent(null, frontmatter);
  ...
  private async applyParentLink(file: TFile, parentFile: TFile): Promise<void> {
    ...
    // User requested format: "[[Folder/Filename]]" (Full path, no extension)
    let linkPath = parentFile.path;
    if (linkPath.endsWith(`.${parentFile.extension}`)) {
  ...
  private async triggerPostCreationHooks(file: TFile): Promise<void> {
    ...
    try {
      ...
      const gcmPlugin = (this.config.app as any)?.plugins?.plugins?.['tps-global-context-menu'];
  ...
  private scheduleFrontmatterReapply(file: TFile, frontmatter: Record<string, any>): void {
    const payload = { ...frontmatter };
    setTimeout(async () => {
      try {
        const current = this.config.app.vault.getAbstractFileByPath(file.path);
        if (!current || !(current instanceof TFile)) return;
        await this.applyEventFrontmatter(current, payload);
      } catch (err) {
        logger.error("[NewEventService] Failed to re-apply frontmatter", err);
      }
    }, 600);
  }

  private scheduleStatusReapply(file: TFile, status: string): void {
    const desired = status;
    setTimeout(async () => {
      try {
        const current = this.config.app.vault.getAbstractFileByPath(file.path);
        if (!current || !(current instanceof TFile)) return;
        await this.config.app.fileManager.processFrontMatter(current, (fm) => {
          if (fm.status !== desired) {
            fm.status = desired;
          }
        });
      } catch (err) {
        logger.error("[NewEventService] Failed to re-apply status override", err);
      }
    }, 1200);
  }
