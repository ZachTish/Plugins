...
export type DeviceRole = "controller" | "replica";

export class DeviceRoleManager {
    private app: App;
    private storageKey: string;
    private _currentRole: DeviceRole = "replica";
    ...
    private loadRole() {
        ...
        if (stored === "controller" || stored === "replica") {
            this._currentRole = stored;
        } else {
            // New device or undefined state: Default to Passive Replica
            this._currentRole = "replica";
            window.localStorage.setItem(this.storageKey, "replica");
            console.log("[TPS Controller] New device detected. Defaulting to 'replica' (Passive).");
        }
