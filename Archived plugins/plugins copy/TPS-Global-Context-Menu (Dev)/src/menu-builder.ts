  ...
  addToNativeMenu(menu: Menu, files: TFile[]) {
      ...
      menu.addItem((item) => {
        item.setTitle('Write on PDF')
          ...
          .onClick(async () => {
            ...
            if (commands.findCommand('open-with-default-app:open')) {
              commands.executeCommandById('open-with-default-app:open');
              return;
            }
    ...
    menu.addItem((item) => {
      ...
      item.setTitle(archiveLabel)
        ...
        .onClick(async () => {
          await this.archiveFiles(entries.map((entry) => entry.file));
        });
    });
