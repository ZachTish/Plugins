...
export class PropertyRowService {
  ...
  createSelectorRow(entries: any[], prop: any): HTMLElement {
    ...
    this.d.addSafeClickListener(valueEl, (e) => {
      const menu = new Menu();
  ...
  createListRow(entries: any[], prop: any): HTMLElement {
    ...
    const refreshTags = () => {
      ...
      this.d.addSafeClickListener(addBtn, () => this.d.openAddTagModal(entries, prop.key));
  ...
  createDatetimeRow(entries: any[], prop: any): HTMLElement {
    ...
    if (formatted) valueEl.textContent = formatted;
    else {
      valueEl.textContent = "Set date...";
      valueEl.style.color = "var(--text-muted)";
    }

    this.d.addSafeClickListener(valueEl, () => this.d.openScheduledModal(entries, prop.key));
    valueEl.addEventListener("mousedown", (e) => e.stopPropagation());
    row.appendChild(label);
    row.appendChild(valueEl);
    return row;
   ...
   */
  openStatusSubmenu(anchor: HTMLElement, entries: any[], onUpdate?: (newVal: string) => void): void {
    const menu = new Menu();
    const fm = entries[0].frontmatter;
    const currentStatus = typeof fm.status === 'string' ? fm.status.trim() : '';

    const statuses = ['open', 'working', 'blocked', 'wont-do', 'complete'];
   ...
   */
  openPrioritySubmenu(anchor: HTMLElement, entries: any[], onUpdate?: (newVal: string) => void): void {
    const menu = new Menu();
    const fm = entries[0].frontmatter;
