...
export class TPSGlobalContextMenuSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    const getAppearanceModeText = (key: AppearanceSettingKey): string =>
      this.plugin.isAppearanceSettingSynced(key) ? 'Synced across devices.' : 'This device only.';

    const attachAppearanceSyncToggle = (setting: Setting, key: AppearanceSettingKey) => {
      setting.addExtraButton((btn) =>
        btn
          .setIcon(this.plugin.isAppearanceSettingSynced(key) ? 'cloud' : 'monitor')
          .setTooltip(
            this.plugin.isAppearanceSettingSynced(key)
              ? 'Syncing across devices. Click to make device-local.'
              : 'Device-local. Click to sync across devices.'
          )
          .onClick(async () => {
            this.plugin.toggleAppearanceSettingSyncMode(key);
    ...
    new Setting(general)
      ...
      .addToggle((toggle) =>
        toggle.setValue(this.plugin.settings.enableArchiveTagMove).onChange(async (value) => {
          ...
          await this.plugin.saveSettings();
          this.plugin.scheduleArchiveSweep();
        }),
      );
    ...
    new Setting(general)
      ...
      .addText((text) =>
        text
          ...
          .onChange(async (value) => {
            this.plugin.settings.archiveFolderPath = value.trim() || 'System/Archive';
            await this.plugin.saveSettings();
          })
      );
    ...
    const menuTextScaleSetting = new Setting(appearance)
      ...
      .addSlider((slider) =>
        slider
          ...
          .onChange(async (value) => {
            this.plugin.setAppearanceSettingValue('menuTextScale', value / 100);
            await saveAppearance();
          }),
      );
    ...
    const buttonScaleSetting = new Setting(appearance)
      ...
      .addSlider((slider) =>
        slider
          ...
          .onChange(async (value) => {
            this.plugin.setAppearanceSettingValue('buttonScale', value / 100);
            await saveAppearance();
          }),
      );
    ...
    const controlScaleSetting = new Setting(appearance)
      ...
      .addSlider((slider) =>
        slider
          ...
          .onChange(async (value) => {
            this.plugin.setAppearanceSettingValue('controlScale', value / 100);
            await saveAppearance();
          }),
      );
    ...
    const densitySetting = new Setting(appearance)
      ...
      .addSlider((slider) =>
        slider
          ...
          .onChange(async (value) => {
            this.plugin.setAppearanceSettingValue('menuDensity', value / 100);
            await saveAppearance();
          }),
      );
    ...
    const radiusSetting = new Setting(appearance)
      ...
      .addSlider((slider) =>
        slider
          ...
          .onChange(async (value) => {
            this.plugin.setAppearanceSettingValue('menuRadiusScale', value / 100);
            await saveAppearance();
          }),
      );
      ...
      .addDropdown((dropdown) =>
        dropdown
          ...
          .onChange(async (value: 'left' | 'center' | 'right') => {
            this.plugin.setAppearanceSettingValue('liveMenuPosition', value);
            await saveAppearance();
          }),
      );
    ...
    const liveOffsetXSetting = new Setting(appearance)
      ...
      .addSlider((slider) =>
        slider
          ...
          .onChange(async (value) => {
            this.plugin.setAppearanceSettingValue('liveMenuOffsetX', value);
            await saveAppearance();
          }),
      );
    ...
    const liveOffsetYSetting = new Setting(appearance)
      ...
      .addSlider((slider) =>
        slider
          ...
          .onChange(async (value) => {
            this.plugin.setAppearanceSettingValue('liveMenuOffsetY', value);
            await saveAppearance();
          }),
      );
    ...
    const subitemsMarginSetting = new Setting(appearance)
      ...
      .addSlider((slider) =>
        slider
          ...
          .onChange(async (value) => {
            this.plugin.setAppearanceSettingValue('subitemsMarginBottom', value);
            await saveAppearance();
          }),
      );
    ...
    const dailyNavScaleSetting = new Setting(appearance)
      ...
      .addSlider((slider) =>
        slider
          ...
          .onChange(async (value) => {
            this.plugin.setAppearanceSettingValue('dailyNavScale', value / 100);
            await saveAppearance();
          }),
      );
    ...
    const dailyNavOpacitySetting = new Setting(appearance)
      ...
      .addSlider((slider) =>
        slider
          ...
          .onChange(async (value) => {
            this.plugin.setAppearanceSettingValue('dailyNavRestOpacity', value);
            await saveAppearance();
    ...
    const modalWidthSetting = new Setting(appearance)
      ...
      .addSlider((slider) =>
        slider
          ...
          .onChange(async (value) => {
            this.plugin.setAppearanceSettingValue('modalWidth', value);
            await saveAppearance();
          }),
      );
    ...
    const modalHeightSetting = new Setting(appearance)
      ...
      .addSlider((slider) =>
        slider
          ...
          .onChange(async (value) => {
            this.plugin.setAppearanceSettingValue('modalMaxHeightVh', value);
            await saveAppearance();
          }),
      );
    ...
    new Setting(appearance)
      ...
      .addButton((button) =>
        button
          .setButtonText('Reset')
          .onClick(async () => {
            this.plugin.setAppearanceSettingValue('menuTextScale', 1);
            this.plugin.setAppearanceSettingValue('buttonScale', 1);
            this.plugin.setAppearanceSettingValue('controlScale', 1);
            this.plugin.setAppearanceSettingValue('menuDensity', 1);
            this.plugin.setAppearanceSettingValue('menuRadiusScale', 1);
            this.plugin.setAppearanceSettingValue('liveMenuPosition', 'center');
            this.plugin.setAppearanceSettingValue('liveMenuOffsetX', 0);
            this.plugin.setAppearanceSettingValue('liveMenuOffsetY', 0);
            this.plugin.setAppearanceSettingValue('subitemsMarginBottom', 0);
            this.plugin.setAppearanceSettingValue('modalWidth', 520);
            this.plugin.setAppearanceSettingValue('modalMaxHeightVh', 80);
            await saveAppearance();
            this.display();
          }),
      );
    ...
    const normalizeConditionOperator = (type: ViewModeConditionType, operator: unknown): ViewModeConditionOperator => {
      ...
      if (type === 'frontmatter') {
        if (value === 'contains' || value === 'not-equals' || value === 'not-contains' || value === 'exists' || value === 'missing') {
          return value as ViewModeConditionOperator;
        }
        return 'equals';
      }
        ...
        conditions.forEach((condition, conditionIndex) => {
          ...
          const operators = condition.type === 'path'
            ...
            : condition.type === 'frontmatter'
              ? ['equals', 'contains', 'not-equals', 'not-contains', 'exists', 'missing']
    ...
    new Setting(automation).setName('Shift+Click Cancel').setDesc('Mark as [-] instead of [x]').addToggle(t => t.setValue(this.plugin.settings.enableShiftClickCancel).onChange(async v => { this.plugin.settings.enableShiftClickCancel = v; await this.plugin.saveSettings(); }));
