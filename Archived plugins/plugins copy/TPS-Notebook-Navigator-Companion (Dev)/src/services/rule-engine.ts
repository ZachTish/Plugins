  ...
  getValuesForConditionSource(source: RuleConditionSource, context: RuleEvaluationContext, field: string): string[] {
    ...
    if (source === "backlink") {
      ...
      if (!targetField) {
        ...
        console.log(`[RuleEngine] All backlink values (no field):`, result);
        return result;
      }
      ...
      const validPaths = allBacklinks.filter(sourcePath => {
        ...
        return cache.frontmatterLinks.some(link => {
          ...
          // Debug match logic
          console.log(`[RuleEngine] Checking link from ${sourcePath}: Key=${link.key}, Link=${link.link}, Dest=${dest?.path}, Match=${isMatch}`);
      ...
      const result = Array.from(values);
      console.log(`[RuleEngine] Backlink values for field '${targetField}':`, result);
      return result;
    }
