...
export class NotebookNavigatorCompanionSettingTab extends PluginSettingTab {
  ...
  private renderGeneralSettings(container: HTMLElement): void {
    ...
    new Setting(container)
      .setName("Apply on startup")
      .setDesc("Walk the vault and normalize metadata after plugin load.")
      .addToggle((toggle) => {
        toggle
          .setValue(this.plugin.settings.applyOnStartup)
          .onChange(async (value) => {
            this.plugin.settings.applyOnStartup = value;
            await this.plugin.saveSettings();
          });
      });
    ...
    new Setting(container)
      ...
      .setDesc("Delay before startup vault walk begins.")
      .addText((text) => {
        text.setPlaceholder("800");
        this.bindCommittedText(text, String(this.plugin.settings.startupDelayMs), async (value) => {
          const parsed = Number.parseInt(value, 10);
          this.plugin.settings.startupDelayMs = Number.isFinite(parsed)
            ? Math.max(0, Math.min(parsed, 30000))
            : 800;
        });
      });
