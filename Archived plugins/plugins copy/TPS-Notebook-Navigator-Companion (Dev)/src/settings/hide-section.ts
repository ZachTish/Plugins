...
function setIcon(el: HTMLElement, iconId: string) {
    // Simple shim for obsidian.setIcon if not globally available, but normally it is imported from "obsidian"
    // However, I imported setIcon from "obsidian" at the top.
    import("obsidian").then(obs => obs.setIcon(el, iconId));
}
