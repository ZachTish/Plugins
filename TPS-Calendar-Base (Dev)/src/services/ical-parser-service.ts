                ...
                try {
                    ...
                    if (isRecurringMaster) {
                        ...
                        while ((next = iterator.next())) {
                            ...
                            // START MODIFICATION - Stable ID for Expanded Occurrence
                            const occurrenceId = `${uid}-${startDate.getTime()}`;
                        ...
                        if (event.recurrenceId) {
                            ...
                            // If this UID is shared by multiple single events (e.g. duplicated in Apple Calendar),
                            // we suffix the ID with its start timestamp so they aren't incorrectly merged/deleted.
                            const duplicateCount = singleEventUidCounts.get(uid) || 1;
                            if (duplicateCount > 1) {
                                stableId = `${uid}-dup-${startDate.getTime()}`;
                            } else {
                                stableId = uid;
                            }
                        }
