...
export class BulkEditService {
    ...
    async applyToFiles(files: TFile[], callback: (fm: Record<string, unknown>, file: TFile) => void): Promise<number> {
        let count = 0;
    ...
    private frontmatterReferencesSeriesTemplate(frontmatter: any, seriesBaseName: string): boolean {
        const rawLink = String(frontmatter?.recurrenceTemplate ?? frontmatter?.recurrencetemplate ?? '').trim().toLowerCase();
        if (!rawLink) return false;
        return rawLink.includes(`[[${seriesBaseName.toLowerCase()}]]`);
    }
    ...
    private async bootstrapTemplateInstanceFromToday(templateFile: TFile, frontmatter: any): Promise<boolean> {
        ...
        const existingInstances = this.plugin.app.vault.getMarkdownFiles().filter((candidate) => {
            ...
            return this.frontmatterReferencesSeriesTemplate(fm, seriesBaseName);
        });
        ...
        const dateStr = window.moment(firstOccurrence).format('YYYY-MM-DD');
